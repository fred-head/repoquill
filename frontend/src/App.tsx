...
export function SettingsDialog({ mode = 'settings', authMode = 'disabled', runningVersion = 'dev', onLoggedOut = () => undefined, autoLockMinutes, onAutoLockMinutes, syncPreferences = defaultSyncPreferences, onSyncPreferences = () => undefined, onNotebookAdded, onClose }: { mode?: 'settings' | 'onboarding'; authMode?:'local'|'disabled'; runningVersion?:string; onLoggedOut?:()=>void; autoLockMinutes: AutoLockMinutes; onAutoLockMinutes: (value: AutoLockMinutes) => void; syncPreferences?: SyncPreferences; onSyncPreferences?: (value: SyncPreferences) => void; onNotebookAdded?: () => Promise<void> | void; onClose: () => void }) {
  ...
  function repositoryURLIssue(value:string){const trimmed=value.trim();if(!trimmed)return '';if(trimmed.includes('\\'))return 'Remove the backslash from the repository address.';if(/^\[.+\]\(https?:\/\//i.test(trimmed))return 'Paste only the repository address, not a Markdown link.';if(/\/(tree|blob)\//.test(trimmed))return 'Use the repository clone address, not a browser page for a branch or file.';if(/^https?:\/\//i.test(trimmed))return 'For private notebooks, copy the SSH clone address shown by your Git service.';if(!/^(ssh:\/\/|[^@\s]+@[^:\s]+:).+/.test(trimmed))return 'Use an SSH clone address such as git@host:owner/notes.git.';return ''}
  ...
  function connectionAction(){const state=connectionResult?.state;if(state==='authentication_failed')return 'Confirm that the public key was added with write access, then test again.';if(state==='host_verification_failed')return 'Review and approve the server fingerprint below.';if(state==='repository_not_found')return 'Check that the repository exists and that the address is correct.';if(state==='branch_not_found'||state==='invalid_branch')return 'Go back and correct the branch, or leave it empty to use the default.';if(state==='network_failed')return 'Check DNS, firewall, and internet access from the RepoQuill server.';return connectionResult&&state!=='success'?'The completed name, address, and generated key are retained. Correct the failed step and test again.':''}
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="settings-title" className="flex max-h-[90vh] w-full max-w-lg flex-col rounded-xl border border-zinc-700 bg-zinc-900 shadow-2xl">
        ...
        <div className={`overflow-y-auto p-5 ${mode === 'settings' ? 'flex flex-col' : ''}`}>
          {mode === 'onboarding' && <section>
            ...
            <form data-step={onboardingStep} className={`${onboardingStep===1?'hidden':'notebook-wizard mt-3 space-y-3'}`} onSubmit={(event) => void cloneNotebook(event)}>
              ...
              {onboardingStep===2&&<p className="text-[11px] text-zinc-500">Example: <code>{provider?providerDetails[provider].example:'git@example.com:user/notes.git'}</code></p>}
              ...
              {onboardingStep===3&&gitAuthType==='managed-ssh'&&managedKey&&provider&&<a href={providerDetails[provider].instructions} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center rounded px-2 text-xs text-amber-300 hover:bg-zinc-800">Open setup instructions</a>}
              {onboardingStep===4&&connectionAction()&&<p className="text-xs text-amber-300">{connectionAction()}</p>}
              {onboardingStep===5&&<div className="rounded-md border border-zinc-800 p-3"><h4 className="text-sm font-medium">Review</h4><dl className="mt-3 space-y-2 text-xs"><div><dt className="text-zinc-500">Notebook</dt><dd>{notebookName}</dd></div><div><dt className="text-zinc-500">Service</dt><dd>{provider?providerDetails[provider].name:'Git server'}</dd></div><div><dt className="text-zinc-500">Host</dt><dd>{repositoryHost(repositoryURL)}</dd></div><div><dt className="text-zinc-500">Repository address</dt><dd className="break-all">{repositoryURL}</dd></div><div><dt className="text-zinc-500">Branch</dt><dd>{repositoryBranch||'Remote default branch'}</dd></div><div><dt className="text-zinc-500">Credential</dt><dd>{gitAuthType==='managed-ssh'?'Dedicated RepoQuill-managed SSH key':'Existing server SSH configuration'}</dd></div></dl><p className="mt-3 text-xs leading-5 text-zinc-500">The notebook opens after cloning. Saved changes remain distinct from synchronization with the connected service.</p></div>}
              ...
              <div className="flex flex-wrap justify-between gap-2 border-t border-zinc-800 pt-3"><button type="button" onClick={()=>setOnboardingStep(step=>Math.max(1,step-1))} className="min-h-10 rounded border border-zinc-700 px-3 text-xs">Back</button>{onboardingStep<5?<button type="button" disabled={(onboardingStep===2&&(!notebookName||!!repositoryURLIssue(repositoryURL)))||(onboardingStep===3&&gitAuthType==='managed-ssh'&&!managedKey)||(onboardingStep===4&&connectionResult?.state!=='success')} onClick={()=>setOnboardingStep(step=>step+1)} className="min-h-10 rounded bg-amber-500 px-4 text-xs font-medium text-zinc-950 disabled:opacity-40">Continue</button>:<button type="submit" disabled={cloneBusy||connectionResult?.state!=='success'} className="min-h-10 rounded bg-amber-500 px-4 text-xs font-medium text-zinc-950 disabled:opacity-40">{cloneBusy?'Cloning…':'Connect notebook'}</button>}</div>
            </form>
          </section>}
